...
-- This migration creates the essential Sprint 2 tables: rules, templates, orders, payments, and traces
-- Run after: 0001_initial_sprint1_schema.sql
...
COMMENT ON TABLE conversation_rules IS 'Automation rules for WhatsApp conversations';
COMMENT ON TABLE templates IS 'Message templates for automated responses';
COMMENT ON TABLE orders IS 'Customer orders from WhatsApp conversations';
COMMENT ON TABLE payment_attempts IS 'Payment attempts for orders (MPESA, etc.)';
COMMENT ON TABLE processing_traces IS 'Processing traces for debugging message processing';
COMMENT ON TABLE analytics_events IS 'Analytics events for tracking system usage';
